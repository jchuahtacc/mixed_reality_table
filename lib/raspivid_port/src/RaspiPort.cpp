...
namespace raspivid {
    ...
    MMAL_STATUS_T RaspiPort::add_callback(RaspiCallback *cb_instance) {
        ...
        if (!pool) {
            vcos_log_error("RaspiPort::enable(): unable to create buffer header pool");
        }
        int queue_length = mmal_queue_length(pool->queue);
        for (int i = 0; i < queue_length; i++) {
            MMAL_BUFFER_HEADER_T *buffer = mmal_queue_get(pool->queue);
            if (!buffer) {
                vcos_log_error("RaspiPort:enable(): unable to get buffer from pool");
            }
            if (mmal_port_send_buffer(port, buffer) != MMAL_SUCCESS) {
                vcos_log_error("RaspiPort::enable(): unable to send buffer to output port");
            }
        }
